...
def phase_4_final_consolidation(containers):
    """
    Giai đoạn 4 (Logic v5.0 - Displacement Optimization):
    Tối ưu hóa bằng cách dồn container cuối cùng (lãng phí) vào các container khác.
    
    Chiến lược:
    Bước 1: Kiểm tra tổng thể xem "dung lượng rỗng" của toàn bộ hệ thống có đủ chứa container lãng phí không.
    Bước 2: Thực hiện dồn dịch:
       - TH1 (Simple): Xếp trực tiếp các pallet từ container lãng phí vào chỗ trống.
       - TH2 (Displacement): Rút bớt pallet nặng từ container "gần đầy" (Hub) sang container rỗng (Relief)
         để tạo chỗ trống cho container lãng phí chui vào Hub.
    """
    print("\n" + "="*80)
    print("BẮT ĐẦU GIAI ĐOẠN 4: TỐI ƯU HÓA HỢP NHẤT (LOGIC DỊCH CHUYỂN TRỌNG LƯỢNG)")
    print("="*80)

    MAX_WEIGHT_LIMIT = 24500.0  # Giới hạn trọng lượng (có thể linh hoạt nhẹ)
    MAX_PALLETS_LIMIT = 20      # Giới hạn số dòng PKL
    EPSILON = 1e-6

    while len(containers) > 1:
        # 1. Xác định Container lãng phí (Waste Container) - thường là cái cuối cùng
        waste_container = containers[-1]
        other_containers = containers[:-1]
        
        print(f"\n--- Đánh giá Container lãng phí: {waste_container.id} ---")
        print(f"    (Load: {waste_container.total_logical_pallets} dòng, {waste_container.total_weight:.2f} kg)")

        # --- BƯỚC 1: KIỂM TRA TỔNG QUAN (GLOBAL FEASIBILITY) ---
        total_free_lines = sum(MAX_PALLETS_LIMIT - c.total_logical_pallets for c in other_containers)
        total_free_weight = sum(MAX_WEIGHT_LIMIT - c.total_weight for c in other_containers)

        print(f"    [System Capacity] Trống: {total_free_lines} dòng, {total_free_weight:.2f} kg")

        # Điều kiện: Tổng chỗ trống phải >= Tổng nhu cầu của container lãng phí
        if total_free_lines < waste_container.total_logical_pallets or \
           total_free_weight < waste_container.total_weight:
            print("    -> KHÔNG ĐỦ CHỖ TRỐNG TỔNG THỂ. Dừng tối ưu hóa.")
            break
        
        print("    -> ĐỦ ĐIỀU KIỆN TỔNG QUAN. Tiến hành thử nghiệm dồn dịch.")

        # --- BƯỚC 2: THỰC HIỆN TỐI ƯU HÓA (2 TRƯỜNG HỢP) ---
        optimization_success = False
        
        # === TH1: XẾP BÌNH THƯỜNG (DIRECT PACKING) ===
        # Thử xếp tất cả pallet của waste_container vào các chỗ trống hiện có mà không cần dịch chuyển
        print("\n   [TH1] Thử xếp trực tiếp (Simple Packing)...")
        
        # Tạo bản sao để mô phỏng
        sim_others_th1 = copy.deepcopy(other_containers)
        sim_waste_pallets = list(waste_container.pallets)
        th1_success = True

        # Sắp xếp pallet cần xếp từ lớn đến bé (ưu tiên pallet khó trước)
        sim_waste_pallets.sort(key=lambda p: p.logical_pallet_count, reverse=True)

        for p in sim_waste_pallets:
            placed = False
            # Tìm chỗ trong các container khác
            # Ưu tiên container còn trống ít nhất (Best Fit) để lấp đầy triệt để
            sorted_hosts = sorted(sim_others_th1, key=lambda c: c.total_quantity, reverse=True)
            
            for host in sorted_hosts:
                # Kiểm tra thủ công các giới hạn
                cond_lines = host.total_logical_pallets + p.logical_pallet_count <= MAX_PALLETS_LIMIT
                cond_weight = host.total_weight + p.total_weight <= MAX_WEIGHT_LIMIT + EPSILON
                
                if cond_lines and cond_weight:
                    host.add_pallet(p)
                    placed = True
                    break
        ...
        if th1_success:
            print("    -> TH1 THÀNH CÔNG! Áp dụng thay đổi.")
            containers = sim_others_th1 # Cập nhật danh sách chính
            optimization_success = True
            continue # Quay lại đầu vòng lặp while để xử lý container tiếp theo (nếu có)

        print("    -> TH1 Thất bại. Chuyển sang TH2.")

        # === TH2: DỊCH CHUYỂN TRỌNG LƯỢNG (DISPLACEMENT LOGIC) ===
        # Logic: Tìm Container "Hub" (gần đầy trọng lượng) -> Rút pallet nặng ra đưa sang Container "Relief" (còn dòng)
        # -> Nhét Waste Container vào Hub.
        print("\n   [TH2] Thử logic dịch chuyển (Displacement)...")

        # 1. Chọn HUB Container: Là container trong other_containers đầy nhất về khối lượng
        # (nhưng chưa đầy về số dòng - hoặc có thể đầy dòng nhưng ta sẽ rút bớt ra)
        sorted_hubs = sorted(other_containers, key=lambda c: c.total_weight, reverse=True)
        
        th2_found_solution = False
        temp_containers_result = [] # Để lưu kết quả nếu thành công

        for hub in sorted_hubs:
            if th2_found_solution: break

            # Hub không được là chính nó (đã tách ở trên)
            # Tìm các Relief Containers (các container khác trừ Hub và Waste)
            relief_containers = [c for c in other_containers if c.id != hub.id]
            if not relief_containers: continue

            # 2. Chọn Pallet để rút ra từ Hub (Highest Density)
            # Công thức: Pallet có trọng lượng riêng (kg/quantity) lớn nhất
            # Hoặc đơn giản là weight_per_pallet lớn nhất.
            pallets_in_hub = sorted(hub.pallets, key=lambda p: p.weight_per_pallet, reverse=True)
            
            for dense_pallet in pallets_in_hub:
                if th2_found_solution: break
                
                # 3. Xác định lượng cần/có thể rút ra
                # Chúng ta cần rút ra sao cho Relief Container có thể nhận được.
                # Tìm Relief Container tốt nhất (còn nhiều dòng nhất để chứa phần rút ra)
                best_relief = max(relief_containers, key=lambda c: MAX_PALLETS_LIMIT - c.total_logical_pallets)
                
                lines_available_in_relief = MAX_PALLETS_LIMIT - best_relief.total_logical_pallets
                weight_available_in_relief = MAX_WEIGHT_LIMIT - best_relief.total_weight

                if lines_available_in_relief < 1: continue # Không có chỗ để nhận

                # Tính toán lượng (qty) tối đa có thể chuyển từ Hub -> Relief
                # Dựa trên số dòng trống của Relief (Ví dụ: Relief thừa 4 dòng -> chuyển 4 qty)
                # Lưu ý: dense_pallet có thể là số lẻ.
                qty_to_move = min(
                    dense_pallet.quantity, 
                    float(lines_available_in_relief), # Mỗi dòng tương đương 1 qty (xấp xỉ logic)
                    weight_available_in_relief / dense_pallet.weight_per_pallet if dense_pallet.weight_per_pallet > 0 else 0
                )

                # Làm tròn xuống nếu cần đảm bảo pallet nguyên (tùy logic, ở đây giả sử có thể cắt lẻ nếu cần, 
                # nhưng theo ví dụ của bạn là "4 dòng của 7.0", tức là cắt số nguyên)
                qty_to_move = math.floor(qty_to_move) 

                if qty_to_move < EPSILON: continue

                print(f"    [*] Giả định Hub: {hub.id}, Relief: {best_relief.id}, Pallet rút: {dense_pallet.id}, Qty rút: {qty_to_move}")

                # --- MÔ PHỎNG QUÁ TRÌNH DỊCH CHUYỂN ---
                # Tạo bản sao sâu để thử nghiệm
                sim_hub = copy.deepcopy(hub)
                sim_relief = copy.deepcopy(best_relief)
                sim_others_remain = [copy.deepcopy(c) for c in relief_containers if c.id != best_relief.id]
                
                # Thực hiện rút từ Sim Hub
                # Tìm pallet tương ứng trong sim_hub
                sim_p_in_hub = next(p for p in sim_hub.pallets if p.id == dense_pallet.id)
                
                # Tách pallet
                remain_part, move_part = sim_p_in_hub.split(qty_to_move)
                
                # Cập nhật Sim Hub (Xóa cũ, thêm phần còn lại nếu có)
                sim_hub.remove_pallet(sim_p_in_hub)
                if remain_part and remain_part.quantity > EPSILON:
                    sim_hub.add_pallet(remain_part)
                
                # Cập nhật Sim Relief (Thêm phần rút ra)
                sim_relief.add_pallet(move_part)

                # 4. Kiểm tra xem sau khi rút, Hub có đủ chỗ cho Waste Container không?
                # Lưu ý: Waste Container có thể cần được xếp vào Sim Hub HOẶC Sim Relief (nếu còn chỗ)
                # Nhưng mục tiêu chính là Hub vì Hub vừa được giải phóng trọng lượng lớn.
                
                current_sim_system = [sim_hub, sim_relief] + sim_others_remain
                
                # Thử xếp items của Waste vào hệ thống mới này
                sim_waste_items = list(waste_container.pallets)
                all_waste_packed = True
                
                for w_item in sim_waste_items:
                    w_placed = False
                    # Ưu tiên xếp vào Hub trước (vì vừa dọn chỗ)
                    target_check_list = [sim_hub] + sorted([sim_relief] + sim_others_remain, key=lambda c: c.total_quantity, reverse=True)
                    
                    for target in target_check_list:
                         if (target.total_logical_pallets + w_item.logical_pallet_count <= MAX_PALLETS_LIMIT and 
                             target.total_weight + w_item.total_weight <= MAX_WEIGHT_LIMIT + EPSILON):
                             target.add_pallet(w_item)
                             w_placed = True
                             break
                    
                    if not w_placed:
                        all_waste_packed = False
                        break
                
                if all_waste_packed:
                    print(f"    -> TH2 THÀNH CÔNG! Rút {qty_to_move} của {dense_pallet.id} từ {hub.id} sang {best_relief.id}.")
                    # Cập nhật danh sách container thật
                    containers = current_sim_system
                    th2_found_solution = True
                    optimization_success = True
        
        if optimization_success:
            continue # Tiếp tục vòng lặp lớn
        else:
            print("    -> TH2 Thất bại. Không thể tối ưu thêm.")
            break # Thoát vòng lặp chính

    print("\n" + "="*80)
    print(f"KẾT THÚC GIAI ĐOẠN 4. Tổng số container: {len(containers)}")
    print("="*80)
    return containers
